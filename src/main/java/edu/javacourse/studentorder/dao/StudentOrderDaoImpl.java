...
public class StudentOrderDaoImpl implements StudentOrderDao {
    ...
    private static final String INSERT_ORDER = "INSERT INTO jc_student_order(student_order_status, student_order_date, " +
            "h_sur_name, h_given_name, h_patronymic, h_date_of_birth, h_passport_seria, h_passport_number, " +
            "h_passport_date, h_passport_office_id, h_post_index, h_street_code, h_building, " +
            "h_extension, h_apartment, h_university_id, h_student_number, w_sur_name, w_given_name, w_patronymic, w_date_of_birth, " +
    ...
    public Long saveStudentOrder(StudentOrder so) throws DaoException {
        ...
        try (Connection con = getConnection();
            ...
            con.setAutoCommit(false);  // Транзакция. Или выполняем всё или откатываемся командой rollback
    ...
    private void saveChildren(Connection con, StudentOrder so, Long soId) throws SQLException {
        try (PreparedStatement stmt = con.prepareStatement(INSERT_CHILD)) {
            ...
            stmt.executeBatch();  // обновляем сразу весь пакет, а не подному значению
        }
    }
...
//        return getStudentOrdersOneSelect();
    ...
    private void findChildren(Connection con, List<StudentOrder> result) throws SQLException {
        String cl = "(" + result.stream().map(so -> String.valueOf(so.getStudentOrderId())).collect(Collectors
                .joining(",")) + ")";
    ...
    private Child fillChild(ResultSet rs) throws SQLException {
        ...
        ad.setStreet(st);
        ad.setPostCode(rs.getString("c_post_index"));
        ad.setBuilding(rs.getString("c_building"));
        ad.setExtension(rs.getString("c_extension"));
        ad.setApartment(rs.getString("c_apartment"));
        child.setAddress(ad);
